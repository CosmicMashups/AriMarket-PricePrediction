...
function renderChart(type, dates, averagePrices, highPrices, lowPrices) {
    ...
    if (chartCanvas) {
        ...
        const chartWidth = Math.max(dates.length * 25, 400); // Ensure a minimum width of 400px
...
function updateChart() {
    ...
    fetch('../../json/beef_brisket.json')
        ...
        .then((data) => {
            ...
            const beefBrisketFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const beefBrisketFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const beefBrisketFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const beefBrisketFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'beefBrisketFirstDate'
            ).innerText = `As of ${beefBrisketFormattedFirstDate}`;
            document.getElementById(
                'beefBrisketFirstPrice'
            ).innerText = `₱${beefBrisketFirstPrice}/kg`;
            document.getElementById(
                'beefBrisketFirstHighPrice'
            ).innerText = `₱${beefBrisketFirstHighPrice}/kg`;
            document.getElementById(
                'beefBrisketFirstLowPrice'
            ).innerText = `₱${beefBrisketFirstLowPrice}/kg`;
