...
fetch('../../json/bangus.json')
    ...
    .then(data => {
        ...
        const bangusFirstDate = dates[0];
        const bangusFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const bangusFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const bangusFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const bangusFormattedFirstDate = formatDate(bangusFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('bangusFirstDate').innerText = `As of ${bangusFormattedFirstDate}`;
        document.getElementById('bangusFirstPrice').innerText = `₱${bangusFirstPrice}`;
        document.getElementById('bangusFirstHighPrice').innerText = `₱${bangusFirstHighPrice}`;
        document.getElementById('bangusFirstLowPrice').innerText = `₱${bangusFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('bangusChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const bangusChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(bangusChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
