...
fetch('../../json/papaya.json')
    ...
    .then(data => {
        ...
        const papayaFirstDate = dates[0];
        const papayaFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const papayaFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const papayaFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const papayaFormattedFirstDate = formatDate(papayaFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('papayaFirstDate').innerText = `As of ${papayaFormattedFirstDate}`;
        document.getElementById('papayaFirstPrice').innerText = `₱${papayaFirstPrice}`;
        document.getElementById('papayaFirstHighPrice').innerText = `₱${papayaFirstHighPrice}`;
        document.getElementById('papayaFirstLowPrice').innerText = `₱${papayaFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('papayaChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const papayaChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(papayaChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
