...
fetch('../../json/garlic.json')
    ...
    .then(data => {
        ...
        const garlicFirstDate = dates[0];
        const garlicFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const garlicFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const garlicFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const garlicFormattedFirstDate = formatDate(garlicFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('garlicFirstDate').innerText = `As of ${garlicFormattedFirstDate}`;
        document.getElementById('garlicFirstPrice').innerText = `₱${garlicFirstPrice}`;
        document.getElementById('garlicFirstHighPrice').innerText = `₱${garlicFirstHighPrice}`;
        document.getElementById('garlicFirstLowPrice').innerText = `₱${garlicFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('garlicChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const garlicChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(garlicChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
