...
fetch('../../json/well_milled_rice.json')
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        const wellMilledRiceFirstDate = dates[0];
        const wellMilledRiceFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const wellMilledRiceFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const wellMilledRiceFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const wellMilledRiceFormattedFirstDate = formatDate(wellMilledRiceFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('wellMilledRiceFirstDate').innerText = `As of ${wellMilledRiceFormattedFirstDate}`;
        document.getElementById('wellMilledRiceFirstPrice').innerText = `₱${wellMilledRiceFirstPrice}`;
        document.getElementById('wellMilledRiceFirstHighPrice').innerText = `₱${wellMilledRiceFirstHighPrice}`;
        document.getElementById('wellMilledRiceFirstLowPrice').innerText = `₱${wellMilledRiceFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('wellMilledRiceChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const wellMilledRiceChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(wellMilledRiceChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
