...
fetch('../../json/eggplant.json')
    ...
    .then(data => {
        ...
        const eggplantFirstDate = dates[0];
        const eggplantFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const eggplantFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const eggplantFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const eggplantFormattedFirstDate = formatDate(eggplantFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('eggplantFirstDate').innerText = `As of ${eggplantFormattedFirstDate}`;
        document.getElementById('eggplantFirstPrice').innerText = `₱${eggplantFirstPrice}`;
        document.getElementById('eggplantFirstHighPrice').innerText = `₱${eggplantFirstHighPrice}`;
        document.getElementById('eggplantFirstLowPrice').innerText = `₱${eggplantFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('eggplantChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const eggplantChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(eggplantChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
