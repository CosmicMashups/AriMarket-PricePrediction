...
fetch('../../json/beef_rump.json')
    ...
    .then(data => {
        ...
        const beefRumpFirstDate = dates[0];
        const beefRumpFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const beefRumpFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const beefRumpFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const beefRumpFormattedFirstDate = formatDate(beefRumpFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('beefRumpFirstDate').innerText = `As of ${beefRumpFormattedFirstDate}`;
        document.getElementById('beefRumpFirstPrice').innerText = `₱${beefRumpFirstPrice}`;
        document.getElementById('beefRumpFirstHighPrice').innerText = `₱${beefRumpFirstHighPrice}`;
        document.getElementById('beefRumpFirstLowPrice').innerText = `₱${beefRumpFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('beefRumpChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const beefRumpChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(beefRumpChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
