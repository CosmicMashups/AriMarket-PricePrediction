...
fetch('../../json/tilapia.json')
    ...
    .then(data => {
        ...
        const tilapiaFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const tilapiaFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const tilapiaFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const tilapiaFormattedFirstDate = formatDate(tilapiaFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('tilapiaFirstDate').innerText = `As of ${tilapiaFormattedFirstDate}`;
        document.getElementById('tilapiaFirstPrice').innerText = `₱${tilapiaFirstPrice}`;
        document.getElementById('tilapiaFirstHighPrice').innerText = `₱${tilapiaFirstHighPrice}`;
        document.getElementById('tilapiaFirstLowPrice').innerText = `₱${tilapiaFirstLowPrice}`;
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(tilapiaChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
