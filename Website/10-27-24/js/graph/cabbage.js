...
fetch('../../json/cabbage.json')
    ...
    .then(data => {
        ...
        const cabbageFirstDate = dates[0];
        const cabbageFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const cabbageFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const cabbageFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const cabbageFormattedFirstDate = formatDate(cabbageFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('cabbageFirstDate').innerText = `As of ${cabbageFormattedFirstDate}`;
        document.getElementById('cabbageFirstPrice').innerText = `₱${cabbageFirstPrice}`;
        document.getElementById('cabbageFirstHighPrice').innerText = `₱${cabbageFirstHighPrice}`;
        document.getElementById('cabbageFirstLowPrice').innerText = `₱${cabbageFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('cabbageChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const cabbageChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(cabbageChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
