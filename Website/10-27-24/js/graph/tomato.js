...
fetch('../../json/tomato.json')
    ...
    .then(data => {
        ...
        const tomatoFirstDate = dates[0];
        const tomatoFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const tomatoFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const tomatoFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        document.getElementById('tomatoFirstDate').innerText = `As of ${tomatoFormattedFirstDate}`;
        document.getElementById('tomatoFirstPrice').innerText = `₱${tomatoFirstPrice}`;
        document.getElementById('tomatoFirstHighPrice').innerText = `₱${tomatoFirstHighPrice}`;
        document.getElementById('tomatoFirstLowPrice').innerText = `₱${tomatoFirstLowPrice}`;
        ...
        // Create the chart
        const tomatoChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(tomatoChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
