...
fetch('../../json/banana_lakatan.json')
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        const bananaLakatanFirstDate = dates[0];
        const bananaLakatanFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const bananaLakatanFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const bananaLakatanFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const bananaLakatanFormattedFirstDate = formatDate(bananaLakatanFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('bananaLakatanFirstDate').innerText = `As of ${bananaLakatanFormattedFirstDate}`;
        document.getElementById('bananaLakatanFirstPrice').innerText = `₱${bananaLakatanFirstPrice}`;
        document.getElementById('bananaLakatanFirstHighPrice').innerText = `₱${bananaLakatanFirstHighPrice}`;
        document.getElementById('bananaLakatanFirstLowPrice').innerText = `₱${bananaLakatanFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('bananaLakatanChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const bananaLakatanChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(bananaLakatanChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
