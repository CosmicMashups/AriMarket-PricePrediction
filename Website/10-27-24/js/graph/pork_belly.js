...
fetch('../../json/pork_belly.json')
    ...
    .then(data => {
        ...
        const porkBellyFirstDate = dates[0];
        const porkBellyFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const porkBellyFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const porkBellyFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const porkBellyFormattedFirstDate = formatDate(porkBellyFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('porkBellyFirstDate').innerText = `As of ${porkBellyFormattedFirstDate}`;
        document.getElementById('porkBellyFirstPrice').innerText = `₱${porkBellyFirstPrice}`;
        document.getElementById('porkBellyFirstHighPrice').innerText = `₱${porkBellyFirstHighPrice}`;
        document.getElementById('porkBellyFirstLowPrice').innerText = `₱${porkBellyFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('porkBellyChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const porkBellyChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(porkBellyChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
