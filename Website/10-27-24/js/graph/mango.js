...
fetch('../../json/mango.json')
    ...
    .then(data => {
        ...
        const mangoFirstDate = dates[0];
        const mangoFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const mangoFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const mangoFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const mangoFormattedFirstDate = formatDate(mangoFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('mangoFirstDate').innerText = `As of ${mangoFormattedFirstDate}`;
        document.getElementById('mangoFirstPrice').innerText = `₱${mangoFirstPrice}`;
        document.getElementById('mangoFirstHighPrice').innerText = `₱${mangoFirstHighPrice}`;
        document.getElementById('mangoFirstLowPrice').innerText = `₱${mangoFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('mangoChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const mangoChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(mangoChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
