...
function updateChart() {
    ...
    fetch('../../json/well_milled_rice.json')
        ...
        .then((data) => {
            ...
            const wellMilledRiceFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const wellMilledRiceFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const wellMilledRiceFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const wellMilledRiceFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'wellMilledRiceFirstDate'
            ).innerText = `As of ${wellMilledRiceFormattedFirstDate}`;
            document.getElementById(
                'wellMilledRiceFirstPrice'
            ).innerText = `₱${wellMilledRiceFirstPrice}/kg`;
            document.getElementById(
                'wellMilledRiceFirstHighPrice'
            ).innerText = `₱${wellMilledRiceFirstHighPrice}/kg`;
            document.getElementById(
                'wellMilledRiceFirstLowPrice'
            ).innerText = `₱${wellMilledRiceFirstLowPrice}/kg`;
