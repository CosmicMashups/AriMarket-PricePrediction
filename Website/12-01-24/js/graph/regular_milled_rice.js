...
function renderChart(type, dates, averagePrices, highPrices, lowPrices) {
    ...
    if (type === 'line') {
        ...
        const candlestickData = highPrices.map((high, index) => ({
            ...
            o: averagePrices[index-1],
            h: high,
            l: lowPrices[index],
            c: averagePrices[index]
        }));
...
function updateChart() {
    ...
    fetch('../../json/regular_milled_rice.json')
        ...
        .then((data) => {
            ...
            const regularMilledRiceFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const regularMilledRiceFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const regularMilledRiceFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const regularMilledRiceFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'regularMilledRiceFirstDate'
            ).innerText = `As of ${regularMilledRiceFormattedFirstDate}`;
            document.getElementById(
                'regularMilledRiceFirstPrice'
            ).innerText = `₱${regularMilledRiceFirstPrice}/kg`;
            document.getElementById(
                'regularMilledRiceFirstHighPrice'
            ).innerText = `₱${regularMilledRiceFirstHighPrice}/kg`;
            document.getElementById(
                'regularMilledRiceFirstLowPrice'
            ).innerText = `₱${regularMilledRiceFirstLowPrice}/kg`;
