...
function updateChart() {
    ...
    fetch('../../json/white_potato.json')
        ...
        .then((data) => {
            ...
            const whitePotatoFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const whitePotatoFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const whitePotatoFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const whitePotatoFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'whitePotatoFirstDate'
            ).innerText = `As of ${whitePotatoFormattedFirstDate}`;
            document.getElementById(
                'whitePotatoFirstPrice'
            ).innerText = `₱${whitePotatoFirstPrice}/kg`;
            document.getElementById(
                'whitePotatoFirstHighPrice'
            ).innerText = `₱${whitePotatoFirstHighPrice}/kg`;
            document.getElementById(
                'whitePotatoFirstLowPrice'
            ).innerText = `₱${whitePotatoFirstLowPrice}/kg`;
