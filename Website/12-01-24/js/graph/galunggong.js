...
function updateChart() {
    ...
    fetch('../../json/galunggong.json')
        ...
        .then((data) => {
            ...
            const galunggongFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const galunggongFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const galunggongFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const galunggongFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'galunggongFirstDate'
            ).innerText = `As of ${galunggongFormattedFirstDate}`;
            document.getElementById(
                'galunggongFirstPrice'
            ).innerText = `₱${galunggongFirstPrice}/kg`;
            document.getElementById(
                'galunggongFirstHighPrice'
            ).innerText = `₱${galunggongFirstHighPrice}/kg`;
            document.getElementById(
                'galunggongFirstLowPrice'
            ).innerText = `₱${galunggongFirstLowPrice}/kg`;
