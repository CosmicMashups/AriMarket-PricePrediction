...
function updateChart() {
    ...
    fetch('../../json/banana_lakatan.json')
        ...
        .then((data) => {
            ...
            const bananaLakatanFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const bananaLakatanFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const bananaLakatanFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const bananaLakatanFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'bananaLakatanFirstDate'
            ).innerText = `As of ${bananaLakatanFormattedFirstDate}`;
            document.getElementById(
                'bananaLakatanFirstPrice'
            ).innerText = `₱${bananaLakatanFirstPrice}/kg`;
            document.getElementById(
                'bananaLakatanFirstHighPrice'
            ).innerText = `₱${bananaLakatanFirstHighPrice}/kg`;
            document.getElementById(
                'bananaLakatanFirstLowPrice'
            ).innerText = `₱${bananaLakatanFirstLowPrice}/kg`;
