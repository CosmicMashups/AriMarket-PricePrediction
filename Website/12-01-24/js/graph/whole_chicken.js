...
function updateChart() {
    ...
    fetch('../../json/whole_chicken.json')
        ...
        .then((data) => {
            ...
            const wholeChickenFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const wholeChickenFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const wholeChickenFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const wholeChickenFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'wholeChickenFirstDate'
            ).innerText = `As of ${wholeChickenFormattedFirstDate}`;
            document.getElementById(
                'wholeChickenFirstPrice'
            ).innerText = `₱${wholeChickenFirstPrice}/kg`;
            document.getElementById(
                'wholeChickenFirstHighPrice'
            ).innerText = `₱${wholeChickenFirstHighPrice}/kg`;
            document.getElementById(
                'wholeChickenFirstLowPrice'
            ).innerText = `₱${wholeChickenFirstLowPrice}/kg`;
