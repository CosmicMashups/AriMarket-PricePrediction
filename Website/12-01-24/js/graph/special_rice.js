...
function updateChart() {
    ...
    fetch('../../json/special_rice.json')
        ...
        .then((data) => {
            ...
            const specialRiceFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const specialRiceFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const specialRiceFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const specialRiceFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'specialRiceFirstDate'
            ).innerText = `As of ${specialRiceFormattedFirstDate}`;
            document.getElementById(
                'specialRiceFirstPrice'
            ).innerText = `₱${specialRiceFirstPrice}/kg`;
            document.getElementById(
                'specialRiceFirstHighPrice'
            ).innerText = `₱${specialRiceFirstHighPrice}/kg`;
            document.getElementById(
                'specialRiceFirstLowPrice'
            ).innerText = `₱${specialRiceFirstLowPrice}/kg`;
