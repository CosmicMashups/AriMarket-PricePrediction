...
function updateChart() {
    ...
    fetch('../../json/premium_rice.json')
        ...
        .then((data) => {
            ...
            const premiumRiceFirstDate = todayIndex !== -1 ? dates[todayIndex] : dates[0];
            const premiumRiceFirstPrice =
                todayIndex !== -1
                    ? Math.round(averagePrices[todayIndex] * 100) / 100
                    : Math.round(averagePrices[0] * 100) / 100;
            const premiumRiceFirstHighPrice =
                todayIndex !== -1
                    ? Math.round(highPrices[todayIndex] * 100) / 100
                    : Math.round(highPrices[0] * 100) / 100;
            const premiumRiceFirstLowPrice =
                todayIndex !== -1
            ...
            document.getElementById(
                'premiumRiceFirstDate'
            ).innerText = `As of ${premiumRiceFormattedFirstDate}`;
            document.getElementById(
                'premiumRiceFirstPrice'
            ).innerText = `₱${premiumRiceFirstPrice}/kg`;
            document.getElementById(
                'premiumRiceFirstHighPrice'
            ).innerText = `₱${premiumRiceFirstHighPrice}/kg`;
            document.getElementById(
                'premiumRiceFirstLowPrice'
            ).innerText = `₱${premiumRiceFirstLowPrice}/kg`;
