...
fetch('../../json/galunggong.json')
    ...
    .then(data => {
        ...
        const galunggongFirstDate = dates[0];
        const galunggongFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const galunggongFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const galunggongFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const galunggongFormattedFirstDate = formatDate(galunggongFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('galunggongFirstDate').innerText = `As of ${galunggongFormattedFirstDate}`;
        document.getElementById('galunggongFirstPrice').innerText = `₱${galunggongFirstPrice}`;
        document.getElementById('galunggongFirstHighPrice').innerText = `₱${galunggongFirstHighPrice}`;
        document.getElementById('galunggongFirstLowPrice').innerText = `₱${galunggongFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('galunggongChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const galunggongChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(galunggongChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
