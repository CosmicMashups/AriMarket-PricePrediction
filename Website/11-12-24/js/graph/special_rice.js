...
fetch('../../json/special_rice.json')
    ...
    .then(data => {
        ...
        const specialRiceFirstDate = dates[0];
        const specialRiceFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const specialRiceFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const specialRiceFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const specialRiceFormattedFirstDate = formatDate(specialRiceFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('specialRiceFirstDate').innerText = `As of ${specialRiceFormattedFirstDate}`;
        document.getElementById('specialRiceFirstPrice').innerText = `₱${specialRiceFirstPrice}`;
        document.getElementById('specialRiceFirstHighPrice').innerText = `₱${specialRiceFirstHighPrice}`;
        document.getElementById('specialRiceFirstLowPrice').innerText = `₱${specialRiceFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('specialRiceChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const specialRiceChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(specialRiceChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
