...
fetch('../../json/red_onion.json')
    ...
    .then(data => {
        ...
        const onionFirstDate = dates[0];
        const onionFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const onionFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const onionFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const onionFormattedFirstDate = formatDate(onionFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('onionFirstDate').innerText = `As of ${onionFormattedFirstDate}`;
        document.getElementById('onionFirstPrice').innerText = `₱${onionFirstPrice}`;
        document.getElementById('onionFirstHighPrice').innerText = `₱${onionFirstHighPrice}`;
        document.getElementById('onionFirstLowPrice').innerText = `₱${onionFirstLowPrice}`;
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(onionChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
