...
fetch('../../json/premium_rice.json')
    ...
    .then(data => {
        ...
        const premiumRiceFirstDate = dates[0];
        const premiumRiceFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const premiumRiceFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const premiumRiceFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const premiumRiceFormattedFirstDate = formatDate(premiumRiceFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('premiumRiceFirstDate').innerText = `As of ${premiumRiceFormattedFirstDate}`;
        document.getElementById('premiumRiceFirstPrice').innerText = `₱${premiumRiceFirstPrice}`;
        document.getElementById('premiumRiceFirstHighPrice').innerText = `₱${premiumRiceFirstHighPrice}`;
        document.getElementById('premiumRiceFirstLowPrice').innerText = `₱${premiumRiceFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('premiumRiceChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const premiumRiceChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(premiumRiceChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
