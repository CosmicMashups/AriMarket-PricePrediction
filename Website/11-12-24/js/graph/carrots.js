...
fetch('../../json/carrots.json')
    ...
    .then(data => {
        ...
        const carrotsFirstDate = dates[0];
        const carrotsFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const carrotsFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const carrotsFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const carrotsFormattedFirstDate = formatDate(carrotsFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('carrotsFirstDate').innerText = `As of ${carrotsFormattedFirstDate}`;
        document.getElementById('carrotsFirstPrice').innerText = `₱${carrotsFirstPrice}`;
        document.getElementById('carrotsFirstHighPrice').innerText = `₱${carrotsFirstHighPrice}`;
        document.getElementById('carrotsFirstLowPrice').innerText = `₱${carrotsFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('carrotsChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const carrotsChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(carrotsChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
