...
fetch('../../json/pork_kasim.json')
    ...
    .then(data => {
        ...
        const porkKasimFirstDate = dates[0];
        const porkKasimFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const porkKasimFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const porkKasimFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const porkKasimFormattedFirstDate = formatDate(porkKasimFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('porkKasimFirstDate').innerText = `As of ${porkKasimFormattedFirstDate}`;
        document.getElementById('porkKasimFirstPrice').innerText = `₱${porkKasimFirstPrice}`;
        document.getElementById('porkKasimFirstHighPrice').innerText = `₱${porkKasimFirstHighPrice}`;
        document.getElementById('porkKasimFirstLowPrice').innerText = `₱${porkKasimFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('porkKasimChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const porkKasimChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(porkKasimChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
