...
fetch('../../json/whole_chicken.json')
    ...
    .then(data => {
        ...
        const wholeChickenFirstDate = dates[0];
        const wholeChickenFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const wholeChickenFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const wholeChickenFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const wholeChickenFormattedFirstDate = formatDate(wholeChickenFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('wholeChickenFirstDate').innerText = `As of ${wholeChickenFormattedFirstDate}`;
        document.getElementById('wholeChickenFirstPrice').innerText = `₱${wholeChickenFirstPrice}`;
        document.getElementById('wholeChickenFirstHighPrice').innerText = `₱${wholeChickenFirstHighPrice}`;
        document.getElementById('wholeChickenFirstLowPrice').innerText = `₱${wholeChickenFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('wholeChickenChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const wholeChickenChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(wholeChickenChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
