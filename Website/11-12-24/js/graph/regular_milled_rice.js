...
fetch('../../json/regular_milled_rice.json')
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        const regularMilledRiceFirstDate = dates[0];
        const regularMilledRiceFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const regularMilledRiceFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const regularMilledRiceFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const regularMilledRiceFormattedFirstDate = formatDate(regularMilledRiceFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('regularMilledRiceFirstDate').innerText = `As of ${regularMilledRiceFormattedFirstDate}`;
        document.getElementById('regularMilledRiceFirstPrice').innerText = `₱${regularMilledRiceFirstPrice}`;
        document.getElementById('regularMilledRiceFirstHighPrice').innerText = `₱${regularMilledRiceFirstHighPrice}`;
        document.getElementById('regularMilledRiceFirstLowPrice').innerText = `₱${regularMilledRiceFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('regularMilledRiceChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const regularMilledRiceChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(regularMilledRiceChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
