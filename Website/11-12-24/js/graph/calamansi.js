...
fetch('../../json/calamansi.json')
    ...
    .then(data => {
        ...
        const calamansiFirstDate = dates[0];
        const calamansiFirstPrice = Math.round(averagePrices[0] * 100) / 100;
        const calamansiFirstHighPrice = Math.round(highPrices[0] * 100) / 100;
        const calamansiFirstLowPrice = Math.round(lowPrices[0] * 100) / 100;
        ...
        const calamansiFormattedFirstDate = formatDate(calamansiFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('calamansiFirstDate').innerText = `As of ${calamansiFormattedFirstDate}`;
        document.getElementById('calamansiFirstPrice').innerText = `₱${calamansiFirstPrice}`;
        document.getElementById('calamansiFirstHighPrice').innerText = `₱${calamansiFirstHighPrice}`;
        document.getElementById('calamansiFirstLowPrice').innerText = `₱${calamansiFirstLowPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('calamansiChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const calamansiChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(calamansiChart, newTheme); // Update chart colors dynamically
        });
    })
    .catch(error => {
