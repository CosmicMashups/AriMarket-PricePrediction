...
fetch('../../json/beef_brisket.json')
    ...
    .then(data => {
        ...
        const beefBrisketFirstDate = dates[0];
        const beefBrisketFirstPrice = Math.round(prices[0] * 100) / 100;
        ...
        const formattedBeefBrisketFirstDate = formatDate(beefBrisketFirstDate);

        // Displaying the first values in HTML elements
        document.getElementById('beefBrisketFirstDate').innerText = `As of ${formattedBeefBrisketFirstDate}`;
        document.getElementById('beefBrisketFirstPrice').innerText = `₱${beefBrisketFirstPrice}`;

        // Check if the canvas context is available
        const ctx = document.getElementById('beefBrisketChart').getContext('2d');
        if (!ctx) {
        ...
        // Create the chart
        const beefBrisketChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
                datasets: [{
        ...
        toggleSwitch.addEventListener('change', () => {
            ...
            updateChartTheme(beefBrisketChart, newTheme); // Update chart colors dynamically
        });
    })
